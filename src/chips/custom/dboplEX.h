...
//Chip profile
struct ProfileData {
    Bit16s WaveTable[ 8 * 1024 ];
};
extern const Bit16s WaveROM[ 8 * 1024 ];

struct Chip;
struct Operator;
struct Channel;

typedef Bits ( DBOPLEx::Operator::*VolumeHandler) ( );
typedef Channel* ( DBOPLEx::Channel::*SynthHandler) ( Chip* chip, Bit32u samples, Bit32s* output );
...
struct Handler {
	...
	void Init( Bitu rate );
	explicit Handler(const ProfileData *profile) { chip.profile = profile; }
};
